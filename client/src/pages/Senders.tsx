...
import { SenderProfile, TransportDocument } from "@shared/schema";
...
export default function Senders() {
  ...
  const filteredProfiles = useMemo(() => {
    if (!senderProfiles || !searchTerm.trim()) return senderProfiles;
...
// Componente per la card del profilo mittente con statistiche e transazioni
interface SenderProfileCardProps {
  profile: SenderProfile;
  onEdit: (profile: SenderProfile) => void;
  onDelete: (profile: SenderProfile) => void;
}

function SenderProfileCard({ profile, onEdit, onDelete }: SenderProfileCardProps) {
  const [selectedPeriod, setSelectedPeriod] = useState<string>("1year");
  
  // Fetch transactions for this sender
  const { data: transactions, isLoading: transactionsLoading } = useQuery({
    queryKey: [`/api/sender-profiles/${profile.id}/transactions`],
    queryFn: getQueryFn<TransportDocument[]>({ on401: "throw" }),
  });
  
  // Fetch financials for this sender
  const { data: financials, isLoading: financialsLoading } = useQuery({
    queryKey: [`/api/sender-profiles/${profile.id}/financials`, selectedPeriod],
    queryFn: async ({ queryKey }) => {
      const [_, period] = queryKey;
      const response = await fetch(`/api/sender-profiles/${profile.id}/financials?period=${period}`);
      if (!response.ok) {
                  ...
                  <TableBody>
                    {transactions.map((doc) => (
                      <TableRow key={doc.id}>
                        ...
                        <TableCell className="truncate">{doc.recipientName}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
